...
#Converts and sorts the gene-like TE annotation file
...
#Bedtool add line with ". -1 -1" when no overlap were found. Those lines are removed.
...
#Bedtool add line with ". -1 -1" when no overlap were found. Those lines are removed.
fix2 <- "awk -F'\t' '$11 !=\".\"' ../ParasiTE_output/neighbor_intergenic_TEs_down_round3.bed > ../ParasiTE_output/neighbor_intergenic_TEs_down_round3.clean.bed " 
system(paste(fix2))
...
#Identify all exonic TEs that are covered by minimum 1% of a TE annotation
...
#Found every genes associated to intronic TEs
...
##I changed here
ssites <- "awk 'BEGIN{FS=OFS=\"\t\"}{print $1,$2,$3+1}' ../ParasiTE_output/splicing_sites_start_end.gff > ../ParasiTE_output/pre-splicing_sites_start_end.bed"
system(paste(ssites))
