...
splicing_sites_start_end <- read.table("../ParasiTE_output/splicing_sites_start_end.gff", header = FALSE, sep = '\t', check.names=FALSE,  stringsAsFactors=F)
...
exon_sites <- read.table("../ParasiTE_output/exon_transcript_wo_fp.final.bed", header = FALSE, sep = '\t', check.names=FALSE)
...
candidates_TEs <- subset(candidates_TEs, select=c(1:4))
print(candidates_TEs)
write.table(candidates_TEs,file="../ParasiTE_output/TEcandidates.bed",sep = "\t",row.names=F, col.names=F)

system(paste("sort ../ParasiTE_output/exon_sites_start_end.gff | uniq > ../ParasiTE_output/exon_sites_start_end.uniq.bed &&
...
AS_check <- overview_gene_TE$TE_id %in% AS_TEcandidates_ss$TE_id
overview_gene_TE <- cbind(overview_gene_TE,AS_check)

########################## Adding Potential ATP check analyses

ATP_check <- overview_gene_TE$TE_id %in% ATP_TEcandidates_ss$TE_id
overview_gene_TE <- cbind(overview_gene_TE,ATP_check)

overview_gene_TE <- overview_gene_TE[,c("TE_gene", "Gene_id", "Total_number_transcripts", "Total_transcripts_with_this_TEs", "total_exon_number", "Freq_TE_isoform", "TE_id", "TE_chromosome", "TE_start", "TE_end", "TE_localisation", "method","AS_check", "ATP_check","Alternative_splicing", "Alternative_transcription", "begining", "middle", "ending", "single" )]
...
overview_gene_TE$Alternative_splicing <- ifelse(overview_gene_TE$Freq_TE_isoform==1 & overview_gene_TE$AS_check==FALSE,NA, overview_gene_TE2$Alternative_splicing )
overview_gene_TE$Alternative_splicing <- ifelse(overview_gene_TE$Freq_TE_isoform==1 & overview_gene_TE$AS_check==FALSE,NA, overview_gene_TE2$Alternative_splicing )
overview_gene_TE$Alternative_transcription <- ifelse(overview_gene_TE$TE_localisation=="Intragenic" & overview_gene_TE$Freq_TE_isoform==1 & overview_gene_TE$AS_check==FALSE,NA, overview_gene_TE2$Alternative_transcription )
