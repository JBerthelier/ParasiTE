...
system(paste("cat ../ParasiTE_output/exon_sites_start.gff ../ParasiTE_output/exon_sites_end.gff > ../ParasiTE_output/exon_sites_start_end.gff"))

ssexon <- "awk 'BEGIN{FS=OFS=\"\t\"}{print $1,$2-1,$3}' ../ParasiTE_output/exon_sites_start_end.gff > ../ParasiTE_output/exon_sites_start_end.bed"
system(paste(ssexon))
...
system(paste("sort ../ParasiTE_output/exon_sites_start_end.bed | uniq > ../ParasiTE_output/exon_sites_start_end.uniq.bed &&
awk '$2!=0 || $3!=0 {print}' ../ParasiTE_output/exon_sites_start_end.uniq.bed > ../ParasiTE_output/pre-exon_sites_start_end.uniq.bed &&
bedtools sort -i ../ParasiTE_output/pre-exon_sites_start_end.uniq.bed > ../ParasiTE_output/exon_sites_start_end.uniq.sorted.bed "))
#the above awk command is to remove lines that contain 0 & 0 has start and end in the files 

system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/TEcandidates.bed -b ../ParasiTE_output/exon_sites_start_end.uniq.sorted.bed > ../ParasiTE_output/TEcandidates_exon_sites.bed"))
ATP_TEcandidates_ss <- read.table("../ParasiTE_output/TEcandidates_exon_sites.bed", header = FALSE, sep = '\t', check.names=FALSE)

ATP_TEcandidates_ss <- ATP_TEcandidates_ss[4]
colnames(ATP_TEcandidates_ss)<-c("TE_id")

write.table(ATP_TEcandidates_ss,file="../ParasiTE_output/Results/Annotations_chimerick_TE_events/Result_TE_candidates_exon_sites.bed",sep = "\t",row.names=F, col.names=F)
...
ATP_check <- detailed_join_gene$TE_id %in% ATP_TEcandidates_ss$TE_id
detailed_join_gene <- cbind(detailed_join_gene,ATP_check)

detailed_join_gene <- detailed_join_gene[,c("isoform_id", "Transcript_id", "Gene_id", "Gene_ref_id", "Total_number_transcripts", "Total_transcripts_with_this_TEs", "Freq_TE_isoform", "FPKM_transcript", "TPM_transcript", "Coverage_transcript", "Coverage_exon", "exon_chromosome", "exon_start", "exon_end", "TE_id", "TE_chromosome", "TE_start", "TE_end", "TE_localisation", "method", "AS_check", "ATP_check", "TE_vs_exon" , "exon_strand", "exon_number", "total_exon_number", "TE_position_isoform", "Alternative_splicing", "Alternative_transcription")]
...
detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$ATP_check==FALSE & detailed_join_gene$Freq_TE_isoform==1,
NA, detailed_join_gene9$Alternative_transcription )
...
detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$AS_check==TRUE & detailed_join_gene$ATP_check==TRUE & (detailed_join_gene$TE_position_isoform=="begining" | detailed_join_gene$TE_position_isoform=="ending" | detailed_join_gene$TE_position_isoform=="single") & detailed_join_gene$Freq_TE_isoform==1 & !is.na(detailed_join_gene$Alternative_transcription) & (detailed_join_gene$total_exon_number=="2"),
NA, detailed_join_gene10$Alternative_transcription )
