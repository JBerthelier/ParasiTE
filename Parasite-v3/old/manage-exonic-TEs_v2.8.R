...
#FOR ANNOTATION OF STRINGTIE R transposons$idexon <- ifelse(grepl("cov",transposons$idexon),transposons$idexon,paste0(transposons$idexon,"cov0.000000;"))
...
##################### Overlapped Exon start check analyses

#check if the analyses TEs are overlaping an exon borders 

exon_sites <- read.table("../ParasiTE_output/exon_transcript_annotation_wo_FP.bed", header = FALSE, sep = '\t', check.names=FALSE)
exon_sites <- exon_sites[1:3]

colnames(exon_sites)<-c("chromosome","start", "end")
exon_sites_start <- exon_sites[,c("chromosome","start", "start")]

write.table(exon_sites_start,file="../ParasiTE_output/exon_sites_start.gff",sep = "\t",row.names=F, col.names=F)

ssexon_start <- "awk 'BEGIN{FS=OFS=\"\t\"}{print $1,$2-1,$3}' ../ParasiTE_output/exon_sites_start.gff > ../ParasiTE_output/exon_sites_start.bed"
system(paste(ssexon_start))

candidates_TEs <- read.table("../ParasiTE_output/Results/Annotations_chimerick_TE_events/Candidate_TEs.bed", header = FALSE, sep = '\t', check.names=FALSE)
candidates_TEs <- subset(candidates_TEs, select=c(1:4))

write.table(candidates_TEs,file="../ParasiTE_output/TEcandidates.bed",sep = "\t",row.names=F, col.names=F)

system(paste("sort ../ParasiTE_output/exon_sites_start.bed | uniq > ../ParasiTE_output/exon_sites_start.uniq.bed &&
awk '$2!=0 || $3!=0 {print}' ../ParasiTE_output/exon_sites_start.uniq.bed > ../ParasiTE_output/pre-exon_sites_start.uniq.bed &&
bedtools sort -i ../ParasiTE_output/pre-exon_sites_start.uniq.bed > ../ParasiTE_output/exon_sites_start.uniq.sorted.bed "))
#the above awk command is to remove lines that contain 0 & 0 has start and end in the files 

system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/TEcandidates.bed -b ../ParasiTE_output/exon_sites_start.uniq.sorted.bed > ../ParasiTE_output/TEcandidates_exon_sites_start.bed"))
ATP_TEcandidates_ss_start <- read.table("../ParasiTE_output/TEcandidates_exon_sites_start.bed", header = FALSE, sep = '\t', check.names=FALSE)

ATP_TEcandidates_ss_start <- ATP_TEcandidates_ss_start[4]
colnames(ATP_TEcandidates_ss_start)<-c("TE_id")

write.table(ATP_TEcandidates_ss_start,file="../ParasiTE_output/Results/Annotations_chimerick_TE_events/Result_TE_candidates_exon_sites_start.bed",sep = "\t",row.names=FALSE, col.names=FALSE)

##################### Overlapped Exon end check analyses
...
colnames(exon_sites)<-c("chromosome","start", "end")
exon_sites_end <- exon_sites[,c("chromosome","end", "end")]

write.table(exon_sites_end,file="../ParasiTE_output/exon_sites_end.gff",sep = "\t",row.names=F, col.names=F)

ssexon_end <- "awk 'BEGIN{FS=OFS=\"\t\"}{print $1,$2-1,$3}' ../ParasiTE_output/exon_sites_end.gff > ../ParasiTE_output/exon_sites_end.bed"
system(paste(ssexon_end))
...
system(paste("sort ../ParasiTE_output/exon_sites_end.bed | uniq > ../ParasiTE_output/exon_sites_end.uniq.bed &&
awk '$2!=0 || $3!=0 {print}' ../ParasiTE_output/exon_sites_end.uniq.bed > ../ParasiTE_output/pre-exon_sites_end.uniq.bed &&
bedtools sort -i ../ParasiTE_output/pre-exon_sites_end.uniq.bed > ../ParasiTE_output/exon_sites_end.uniq.sorted.bed "))
#the above awk command is to remove lines that contain 0 & 0 has start and end in the files 

system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/TEcandidates.bed -b ../ParasiTE_output/exon_sites_end.uniq.sorted.bed > ../ParasiTE_output/TEcandidates_exon_sites_end.bed"))
ATP_TEcandidates_ss_end <- read.table("../ParasiTE_output/TEcandidates_exon_sites_end.bed", header = FALSE, sep = '\t', check.names=FALSE)

ATP_TEcandidates_ss_end <- ATP_TEcandidates_ss_end[4]
colnames(ATP_TEcandidates_ss_end)<-c("TE_id")

write.table(ATP_TEcandidates_ss_end,file="../ParasiTE_output/Results/Annotations_chimerick_TE_events/Result_TE_candidates_exon_sites_end.bed",sep = "\t",row.names=FALSE, col.names=FALSE)
...
ATP_check_start <- detailed_join_gene$TE_id %in% ATP_TEcandidates_ss_start$TE_id
detailed_join_gene <- cbind(detailed_join_gene,ATP_check_start)

########################## Adding Potential exons_borders check analyses
ATP_check_end <- detailed_join_gene$TE_id %in% ATP_TEcandidates_ss_end$TE_id
detailed_join_gene <- cbind(detailed_join_gene,ATP_check_end)

detailed_join_gene <- detailed_join_gene[,c("isoform_id", "Transcript_id", "Gene_id", "Gene_ref_id", "Total_number_transcripts", "Total_transcripts_with_this_TEs", "Freq_TE_isoform", "FPKM_transcript", "TPM_transcript", "Coverage_transcript", "Coverage_exon", "exon_chromosome", "exon_start", "exon_end", "TE_id", "TE_chromosome", "TE_start", "TE_end", "TE_localisation", "method", "AS_check", "ATP_check_start", "ATP_check_end","exon_strand", "exon_number", "total_exon_number", "TE_position_isoform", "Alternative_splicing", "Alternative_transcription")]

###### AS rules (MXE or MSE are simply SE)
...
detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & ( detailed_join_gene$TE_position_isoform=="middle" | detailed_join_gene$TE_position_isoform=="single") & detailed_join_gene$AS_check==TRUE ,NA, detailed_join_gene5$Alternative_transcription )

#Rule 3a
detailed_join_gene9 <- detailed_join_gene
detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$Freq_TE_isoform==1 & ((detailed_join_gene$TE_position_isoform=="ending" & detailed_join_gene$exon_strand=="+" & detailed_join_gene$ATP_check_end==FALSE) | (detailed_join_gene$TE_position_isoform=="begining" & detailed_join_gene$exon_strand=="+" & detailed_join_gene$ATP_check_start==FALSE) | (detailed_join_gene$TE_position_isoform=="ending" & detailed_join_gene$exon_strand=="-" & detailed_join_gene$ATP_check_start==FALSE) | (detailed_join_gene$TE_position_isoform=="begining" & detailed_join_gene$exon_strand=="-" & detailed_join_gene$ATP_check_end==FALSE) | ( detailed_join_gene$ATP_check_start==FALSE & detailed_join_gene$ATP_check_end==FALSE)) ,NA, detailed_join_gene9$Alternative_transcription )

#Rule 3b   ##to decrease more I can add detailed_join_gene$total_exon_number=="2"|detailed_join_gene$total_exon_number=="3"
detailed_join_gene10 <- detailed_join_gene
detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$AS_check==FALSE & ((detailed_join_gene$exon_strand=="+" & detailed_join_gene$TE_position_isoform=="begining" & detailed_join_gene$ATP_check_start==FALSE) | (detailed_join_gene$exon_strand=="+" & detailed_join_gene$TE_position_isoform=="ending" & detailed_join_gene$ATP_check_end==FALSE) | (detailed_join_gene$exon_strand=="-" & detailed_join_gene$TE_position_isoform=="begining" & detailed_join_gene$ATP_check_end==FALSE) | (detailed_join_gene$exon_strand=="-" & detailed_join_gene$TE_position_isoform=="ending" & detailed_join_gene$ATP_check_start==FALSE) |detailed_join_gene$TE_position_isoform=="single" ),NA, detailed_join_gene10$Alternative_transcription )

#Rule 3c
#detailed_join_gene10 <- detailed_join_gene
#detailed_join_gene$Alternative_transcription <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$ATP_check_start==FALSE & detailed_join_gene$ATP_check_end==FALSE & (detailed_join_gene$TE_position_isoform=="begining" | detailed_join_gene$TE_position_isoform=="ending" | detailed_join_gene$TE_position_isoform=="single") & !is.na(detailed_join_gene$Alternative_splicing),NA, detailed_join_gene10$Alternative_transcription)

#Rule 4: off
#detailed_join_gene1 <- detailed_join_gene
#detailed_join_gene$Alternative_splicing <- ifelse(detailed_join_gene$TE_localisation=="Intragenic" & detailed_join_gene$ATP_check==TRUE & detailed_join_gene$AS_check==TRUE ( detailed_join_gene$TE_position_isoform=="begining" | #detailed_join_gene$TE_position_isoform=="ending" | detailed_join_gene$TE_position_isoform=="single") & !is.na(detailed_join_gene$Alternative_transcription), NA, detailed_join_gene1$Alternative_splicing )

#Rule 5:
#detailed_join_gene2 <- detailed_join_gene
#detailed_join_gene$Alternative_splicing <- ifelse(detailed_join_gene$TE_localisation=="Intergenic" & ( detailed_join_gene$TE_position_isoform=="begining" | detailed_join_gene$TE_position_isoform=="ending" | detailed_join_gene$TE_position_isoform=="single") & detailed_join_gene$AS_check==FALSE, NA & !is.na(detailed_join_gene$Alternative_transcription), detailed_join_gene2$Alternative_splicing )

###### Check if exon candidates are overlaping CDS or 5UTR or 3UTR

exon_annotation_candidate <- detailed_join_gene[,c("exon_chromosome", "exon_start", "exon_end", "Transcript_id")]
head(exon_annotation_candidate)
exon_annotation_candidate <- as.data.frame(exon_annotation_candidate)
head(exon_annotation_candidate)
write.table(exon_annotation_candidate,file="../ParasiTE_output/exon_annotation_candidate.bed",sep = "\t",row.names=F,col.names=F)

##check CDS
system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/exon_annotation_candidate.bed -b ../ParasiTE_output/cds_annotation.bed  > ../ParasiTE_output/CDS_results.bed "))

##check 5UTR
system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/exon_annotation_candidate.bed -b ../ParasiTE_output/five_annotation.bed  > ../ParasiTE_output/five_prime_results.bed "))

##check 3UTR
system(paste("bedtools intersect -nonamecheck -wo -a ../ParasiTE_output/exon_annotation_candidate.bed -b ../ParasiTE_output/three_annotation.bed  > ../ParasiTE_output/three_prime_results.bed "))
