...
option_list = list(
    ...
    make_option(c("-I", "--Tintragenic"), type="character", default=0.8, 
              help="Minimum proportion (%) of TE lenght that overlap a gene to be view as intragenic [default= %default]", metavar="number"),
...
if (is.null(opt$Pmode)){
  print_help(opt_parser)
  stop("A mode has to be choose (see -P option", call.=FALSE)
}
...
system(paste("cd ../CATANA/Classification && cat A5SS.gff A3SS.gff SE.gff RI.gff MSE.gff > Alternative_splicing_events.gff &&
